...
def simper(data, factor, spNames=None):
	'''
	The API for ecopy is now functionally dead, as mine will eventually be in the future. This code was corrected by me,
	but I did not check it for efficiency, only plugged some holes to make it run again
	Usage of this (description of function parameters is below, as prepared by the creators):
	
	import SIMPER_func as smp
	simp = smp.simper(distance_matrix, groups_list)
	
	
	Below is the description included by the original creators:
	...
	while i < len(groupIDs)-1:
		...
		while j < len(groupIDs):
			...
			spRat = np.divide(
				spMeans,
				spSds,
				out=np.full_like(spMeans, np.nan),
				where=spSds != 0
			)
			spRat = np.round(spRat, 2)
			total = spMeans.sum()
			if total == 0:
				spPct = np.zeros_like(spMeans)
			else:
				spPct = np.round(spMeans / total * 100, 2)
